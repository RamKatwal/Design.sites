...
## UI Framework: shadcn/ui (MANDATORY)

We use **shadcn/ui** components as the default UI building blocks.

### Rules
- Prefer shadcn/ui components over custom components when an equivalent exists.
- Do NOT introduce other UI libraries (MUI, Chakra, AntD, Mantine, etc.) unless explicitly requested.
- Do NOT re-implement common patterns (Button, Dialog, Dropdown, Tabs, Tooltip, Toast, Sheet, etc.) from scratch.
- If a shadcn component is missing, add it using the shadcn CLI (or implement in the same style) and keep it consistent with existing components.
